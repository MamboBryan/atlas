...
test.runIf(canRun)(
  ...
  async () => {
    ...
    const { data: u } = await c.auth.admin.createUser({
      email: "hiring-sync-admin@atlas.com",
      email_confirm: true,
    });
...
test.runIf(canRun)(
  "hideNames + hiddenColumns: stable anonymous labels; hidden question flagged; hidden answers stored",
  async () => {
    const c = admin!;
    const { data: u } = await c.auth.admin.createUser({
      email: "hiring-sync-admin@atlas.com",
    ...
    for (const cand of cands1)
      expect(cand.display_name).toMatch(/^Candidate \d+$/);
    const labelsByEmail1 = Object.fromEntries(
      cands1.map((cand) => [cand.email, cand.display_name]),
    );

    const questions1 = (
      await c
        .from("evaluation_questions")
        .select("id,column_key,is_hidden")
        .eq("evaluation_id", id)
    ).data!;
    ...
    const hiddenAnswers = (
      await c
        ...
        .select("answer_text")
        .eq("evaluation_id", id)
        .eq("question_id", hiddenQ.id)
        .order("answer_text")
    ).data!;
    expect(hiddenAnswers.map((a) => a.answer_text)).toEqual([
      "Ann Smith",
      "Bob Jones",
    ]);
    ...
    const labelsByEmail2 = Object.fromEntries(
      cands2.map((cand) => [cand.email, cand.display_name]),
    );
