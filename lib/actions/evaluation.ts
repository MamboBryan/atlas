...
export async function createEvaluationAction(
  ...
  const { error: ownerErr } = await svc
...
export async function addEvaluationOwnerAction(
  input: unknown,
): Promise<ActionResult<null>> {
  ...
  await requireEvaluationOwner(parsed.data.evaluationId);
  const svc = atlasServiceClient();
  const { error } = await svc.from("evaluation_owners").upsert(
    {
...
export async function removeEvaluationOwnerAction(
  input: unknown,
): Promise<ActionResult<null>> {
  const parsed = evaluationOwnerInput.safeParse(input);
  if (!parsed.success) return err("invalid_input", parsed.error.message);
  ...
  try {
    ...
    await svc
      ...
      .update({
        ...
        name_column: hideNames ? null : nameColumn,
...
export async function saveEvaluationFieldsAction(
  input: unknown,
): Promise<ActionResult<null>> {
  ...
  const timestampColumn =
    fields.find((f) => f.role === "timestamp")?.column ?? null;
  ...
  const qRows = fields
    ...
    .map((f, i) => ({
      ...
      prompt: f.label,
      position: i,
      is_active: f.role !== "ignore",
      is_hidden: f.role === "context",
    }));
  if (qRows.length) {
    const { error } = await svc
      .from("evaluation_questions")
...
export async function refreshEvaluationAction(input: unknown) {
  ...
  const { data: ev } = await svc
    .from("evaluations")
    .select(
      "sheet_id,sheet_tab,email_column,name_column,timestamp_column,mapping_confirmed,hide_names",
    )
    .eq("id", parsed.data.evaluationId)
    .single();
  if (!ev?.mapping_confirmed || !ev.email_column)
    return err("not_ready", "connect a sheet and confirm mapping first");
  if (!ev.sheet_id) return err("no_sheet", "re-upload the CSV to update");
  try {
    const grid = await readSheet(ev.sheet_id, ev.sheet_tab);
