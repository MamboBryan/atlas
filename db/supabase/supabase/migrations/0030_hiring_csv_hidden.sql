-- CSV import fallback + per-column hidden fields with name-anonymization ("blind" review).

alter table public.evaluations add column hide_names boolean not null default false;
alter table public.evaluation_questions add column is_hidden boolean not null default false;

-- Re-create evaluation_results/evaluation_panel_progress: identical to 0029 except every
-- place a question is joined/filtered by is_active also requires is_hidden = false, so
-- hidden questions are excluded from the per-cell cross join, the distinct-rater floor
-- count, and panel_progress's total/rated.
