...
test.runIf(canRun)("hidden question: answer text withheld from panelists via RLS; excluded from results", async () => {
  const c = admin!;
  const A = await userClient("hiring-rate-a@atlas.com");
  const B = await userClient("hiring-rate-b@atlas.com");
  const D = await userClient("hiring-rate-d@atlas.com");
  ...
  const { data: ev } = await c.from("evaluations").insert({ name: "T", status: "open", created_by: ids[0] }).select("id").single();
  const { data: q1 } = await c.from("evaluation_questions")
    .insert({ evaluation_id: ev!.id, column_key: "Q1", prompt: "Q1", position: 0, is_hidden: false }).select("id").single();
  const { data: qHidden } = await c.from("evaluation_questions")
    .insert({ evaluation_id: ev!.id, column_key: "Full Name", prompt: "Full Name", position: 1, is_hidden: true }).select("id").single();
  const { data: cand } = await c.from("evaluation_candidates")
    .insert({ evaluation_id: ev!.id, email: "cand-hidden@x.com", display_name: "Candidate 1" }).select("id").single();
  await c.from("evaluation_panelists").insert(ids.map((id) => ({ evaluation_id: ev!.id, profile_id: id })));

  // Seed answers on both the visible and hidden question (as syncEvaluation would).
  await c.from("evaluation_answers").insert([
    { evaluation_id: ev!.id, candidate_id: cand!.id, question_id: q1!.id, answer_text: "visible answer" },
    { evaluation_id: ev!.id, candidate_id: cand!.id, question_id: qHidden!.id, answer_text: "Real Name" },
  ]);
  ...
  for (let i = 0; i < 3; i++) {
    await clients[i].from("evaluation_ratings").insert({
      evaluation_id: ev!.id, candidate_id: cand!.id, question_id: q1!.id, rater_id: ids[i], score: i + 3,
    });
    await clients[i].from("evaluation_ratings").insert({
      evaluation_id: ev!.id, candidate_id: cand!.id, question_id: qHidden!.id, rater_id: ids[i], score: 1,
    });
  }

  // (i) RLS: a panelist cannot read the hidden question's answer text directly,
  //     but can still read the visible one.
  const hiddenRead = await A.from("evaluation_answers").select("answer_text").eq("question_id", qHidden!.id);
  expect(hiddenRead.data).toEqual([]);
  const visibleRead = await A.from("evaluation_answers").select("answer_text").eq("question_id", q1!.id);
  ...
  expect(res.suppressed).toBe(false);
  const candOut = res.candidates[0];
  expect(candOut.cells.some((x: any) => x.question_id === qHidden!.id)).toBe(false);
